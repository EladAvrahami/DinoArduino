...
void loop() {
    ...
    myservo.write(unpress_angle);              // set the motor angle to be far from the button
    delay(1);                                 // after indicate changes in light delay 
    if (analogRead(A0) < threshold)             // if the arduino indicates decrement less than 400  
    {
